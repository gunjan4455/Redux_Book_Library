...
describe('all books reducer', () => {
    it.only('reducer should return the initial state', () => {
        expect(reducers.booksReducer(undefined, {})).toEqual(initialState);
    });

    it.only('reducer should return all the books', () => {
        initialState.books = [{
            title: "2 states done",
    ...
    it.only('reducer should return all filtered books', () => {
        initialState.books = [{
            title: "2 states done",
            imageUrl: "2_states.jpg",
            author: "Illiam Cruze",
        ...
        const action = {type: types.GET_FILTERED_BOOKS, books: initialState.books};
        expect(reducers.booksReducer(initialState, action)).toEqual(initialState);
    });

    it.only('reducer should return detail of a particular book', () => {
        initialState.book = {
            title: "2 states done",
            imageUrl: "2_states.jpg",
            author: "Illiam Cruze",
    ...
    it.only('reducer should return value of isSuccess flag: true', () => {
        initialState.isSuccess = true;
        const action = {type: types.CHECK_SUCCESS, check: initialState.isSuccess};
        expect(reducers.booksReducer(initialState, action)).toEqual(initialState);
    });

    it.only('reducer should return value of isSuccess flag: false', () => {
